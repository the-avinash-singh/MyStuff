...
router.post(
  "/creataccount",
  [
    body("name", "Enter a valid name").isLength({ min: 3 }), //here we have set the validations using express validator packege
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      //if all the validations are true then tis data will be send
      const data = {
        user: {
          id: user.id,
        },
